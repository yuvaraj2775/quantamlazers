...
export async function POST(req) {
  ...
  try {
    ...
    const result = await db.run(
        `INSERT INTO itemmaster (name,  description, comments, enabled) VALUES ( ?, ?, ?, ?)`,
        [name, description, comments, true]  // Default to enabled
      );
...
export async function PUT(req) {
    const { id, name,  description, comments, enabled } = await req.json();
    ...
    try {
        ...
        // Update the item
        await db.run(
            `UPDATE itemmaster SET name = ?, description = ?, comments = ?, enabled = ? WHERE id = ?`,
            [name, description, comments, enabled, id]
        );
